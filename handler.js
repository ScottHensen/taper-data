...
module.exports.getAllBands = (event, context, callback) => {
  var parmsGetAll = {
      TableName: process.env.DYNAMODB_BANDS_TABLE
    }
...
module.exports.getOneBand = (event, context, callback) => {
  var parmsGetOne = {
    TableName: process.env.DYNAMODB_BANDS_TABLE,
...
module.exports.getAllShows = (event, context, callback) => {
  var parmsGetAll = {
      TableName: process.env.DYNAMODB_SHOWS_TABLE
    }
  dynamo.scan(parmsGetAll, (error, result) => {
    var response = getResponse(error, result)
    callback(null, response)
